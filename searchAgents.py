...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    cornersLeft = list(self.corners)
    if self.startingPosition in cornersLeft:
      cornersLeft.remove(self.startingPosition)
    cornersLeft.insert(0, self.startingPosition)
    self.startState = tuple(cornersLeft)
  ...
  def isGoalState(self, state):
    ...
    position = stateList[0]
    stateList.pop(0)
    cornersLeft = stateList
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
        ...
        temp = copy.copy(cornersLeft)
        if nextPosition in temp:
          self.reachedCorners[nextPosition] = True
          temp.remove(nextPosition)
        temp.insert(0, nextPosition)
        nextState = tuple(temp)
        cost = 1
        successors.append( ( nextState, action, cost) )
    print state, ' successors are: ', successors    
    # Bookkeeping for display purposes
    self._expanded += 1 
    if state not in self._visited:
      self._visited[state] = True
...
def cornersHeuristic(state, problem):
  ...
  for corner in cornersLeft:
    distance += abs(corner[0] - position[0]) + abs(corner[1] - position[1])
